...
```159:164:codex-rs/tools/src/json_schema.rs
...
整段函数里穿插了 `run_pre_tool_use_hooks` / `run_post_tool_use_hooks` 与 `notify_tool_start` / `notify_tool_finish`（见同文件 11–25 行 use、487 行附近的调用点）。从源码可以观察到：派发函数刻意把 Hook、生命周期与执行体绑定在同一个 `async fn` 里——这能保证一次工具调用在 Hook 视角看是原子的，但也意味着任何一段的延迟都会卡住其他段。
...
真正的复杂度落在 `multi_agents_spec.rs`（837 行）里：参数 schema、agent 类型描述、namespace 组织、v1 / v2 兼容都在那。这种"执行逻辑薄、接口定义厚"的拆法让"做什么"和"长什么样"分离，但代价是跨文件阅读成本上升——读 handler 的时候，必须随时跳到 spec 文件才能完整理解参数语义。
...
v1 namespace（`MULTI_AGENT_V1_NAMESPACE`）与 v2 plain function 并存的阶段是有迁移价值的，但同时引入了文档负担——读者需要分辨"当前在哪个工具面下、参数 schema 该读哪份"。当 v2 稳定后清理 v1 是显而易见的方向，但何时清理涉及对外部依赖者的兼容承诺。
...
- workspace member：`codex-rs/Cargo.toml` 经解析为 **113** 个 crate。
- 关键文件行数（`wc -l`）：
