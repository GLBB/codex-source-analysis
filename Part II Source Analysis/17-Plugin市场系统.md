...
- `codex-rs/plugin/src/lib.rs`（72 行，纯类型层 `PluginId / LoadedPlugin / PluginCapabilitySummary`）

按本章复核口径（2026-05-26，本地源码基线 `/Users/hexiaonan/workspace/formless/refer/codex`）的几个关键数字：

- `codex-rs/core-plugins` crate 源码量：**17,490 行**（含测试），分布在 **24** 个 `.rs` 文件
...
- **远程 bundle 安全栅栏**（`remote_bundle.rs:24-31`）：下载 60 秒超时、单文件 50 MiB 上限、解压后总大小 250 MiB 上限、错误响应体最多 8 KiB、只接受 `https://`，HTTP 仅限调试构建 + loopback
...
把 plugin/lib.rs 单拎出来看，它本身只有 72 行（`plugin/src/lib.rs:1-71`），只定义"对外可见的 plugin 身份与 telemetry 摘要"。真正的市场逻辑全部在 `core-plugins` 这个 crate，这种"瘦类型层 + 厚实现层"的拆分是 Codex 的一贯风格，便于上游 crate（如 telemetry / config）依赖 `plugin` 而无须依赖 `core-plugins` 的网络栈。
...
这张图严格对应 `startup_sync.rs:75-136` 的 `sync_openai_plugins_repo_with_transport_overrides`。关键判断在 `HasLocal`：**如果用户磁盘上已经有一份可用的 curated snapshot，绝对不去走 backup archive 这条路径**。这是一个"宁可让用户暂时拿旧数据，也不要让一个滞后备份覆盖新快照"的工程决定，体现在 `startup_sync.rs:104-110` 的注释里："The export archive is a lagging backup path. Only use it to bootstrap a missing local curated snapshot, never to refresh an existing one."
...
这是为了等 `startup_sync`（curated GitHub clone）先落盘 `~/.codex/.tmp/plugins/.agents/plugins/marketplace.json`，否则远程同步要安装的 plugin 在本地 curated marketplace 里找不到，整次 sync 会以 `LocalMarketplaceNotFound` 失败。50ms 轮询粒度 + 10s 上限 = 最多 200 轮，足够覆盖正常 git clone 用时。
...
Issue #17622 的根本修法应该是把 git 子进程的环境强制设为：
...
整个 crate 1.7 万行、195+ 测试用例、11 字段的 `PluginsManager`、60s 超时 / 50 MiB 下载 / 250 MiB 解压三道栅栏 —— 这些数字共同说明：**Codex 把 plugin marketplace 当成了"和沙箱、hook 一个级别的子系统"来做**，不是简单的扩展面。

但相对成熟度也带来明显代价：从 commit 历史看，2026 年 4 月的 PR #17277 是一次架构级转向（local/remote 不再 sync 而是并存），而 startup_remote_sync 这套 fire-and-forget + marker 文件路径仍然按"老 sync 模型"在跑。这种"新旧模型同时在线"的状态在 1~2 个版本内必然会引发更细的对账问题；社区里关于"装了 plugin 看不到、卸了 plugin 又出现"的零星 issue 大概率源自这一过渡期。
