...
- `hooks` crate 的源码量：**10,514 行**（含测试），其中 `engine/mod_tests.rs` 一个文件就有 1350 行
...
%%{init:{'theme':'neutral','themeVariables':{'background':'#ffffff'}}}%%
flowchart LR
    A["可治理性 / 10 lifecycle events"] -.tension.-> B["可信任性 / trust hash + state"]
    A -.tension.-> C["可观测性 / HookStarted / HookCompleted"]
    B -.tension.-> D["可扩展性 / plugin hooks"]
    C -.tension.-> E["上下文预算 / output spill 2500 tok"]
    D -.tension.-> F["与 Claude Code 兼容"]
    E -.tension.-> F
    A --> G["核心权衡：不让 hook 成为新的攻击面"]
    B --> G
    D --> G
```
...
从实现和 PR #20321 评审讨论看，这更像是一个有意取舍：它牺牲了"脚本被改也要重新审批"的强度，换来"管理员把脚本部署在 managed_dir 后能自由发新版"的可维护性。
...
3. **`PreToolUse` 看不到 transcript 文件直到 transcript 写盘**  
   `transcript_path` 字段是 nullable（schema.rs:278），首轮可能为 null。脚本要做 `if transcript_path:` 判断。
...
6. **企业部署形态** Codex 较突出的是 `requirements.toml` 里的 `allow_managed_hooks_only` + `managed_dir`，这条在 Claude Code 等竞品里没有等价物（codex-docs.com 文档明确）。
...
至于 Codex 与 Claude Code 的关系：源码层面 Codex 是 _careful re-implementation, not a fork_。同样的事件名、相近的 JSON 字段，但 trust 模型、subagent 形态、`turn_id` 扩展、`updatedInput` 落地节奏都是 Codex 自己长出来的肌肉。社区在 2026 年频繁讨论的"Codex hooks 只能 Bash"已经是历史问题，本章源码基线下，apply_patch / MCP 都已经接入；下一个值得关注的点是 PR #15266 提议的 SessionStart eager 时序、`updatedMCPToolOutput` 的正式启用、以及 trust hash 是否会扩展到 hash 脚本内容本体。这些演化方向才决定 Codex hook 能不能从"贴近 Claude 的兼容层"变成"企业级 Agent governance 的重要候选"。
