...
| L3 原始轨迹包 | `codex-rs/rollout-trace`（26 个非测试源码文件，约 8800 行） | 用于事后语义重放的 trace bundle | `$CODEX_ROLLOUT_TRACE_ROOT/trace-<uuid>-<thread>/` | 否 |
| L4 跨会话历史 | `codex-rs/message-history/src/lib.rs`（435 行） | 跨会话的用户消息全集（TUI ↑↓ 历史） | `~/.codex/history.jsonl` | 受 `history.persistence` 控制 |

定量上，会话持久化主线代码约 **1.6 万行 Rust/SQL**（按本章涉及的关键 crate 与 migration 粗略统计），独立于其它 ~120 个 crate；并且这 5 层有非常清晰的依赖方向：
...
4. **跨主机不可发现**：rollout 是绝对路径绑定的，跨主机 resume 必须显式拷贝文件，不能让 SQLite 行成为"幽灵"。这是一个**反向的设计**——可能是为了避免错误的恢复，Codex 让跨主机 resume 显式失败而非默默拼接。
...
更值得注意的是 `sanitize_rollout_item_for_persistence`（`policy.rs:42-63`）：当 `Extended` 模式被启用并准备持久化 `ExecCommandEnd` 时，会把 `aggregated_output` **从中间截断**到 10 000 字节、清空 `stdout/stderr/formatted_output`，避免一行 50 MB 的 `grep -r` 把会话日志撑爆。
...
`LiveThreadInitGuard::drop`（`live_thread.rs:72-87`）特意通过 `tokio::runtime::Handle::try_current()` 在 `Drop` 路径里 `spawn` 异步丢弃任务——因为 `Drop` 本身是同步上下文，但 `discard()` 需要异步 IO。这一段并不优雅（没有 Tokio runtime 就只能 warn），但它是一种可以兼顾"不阻塞、不泄漏"的实现方式。
...
源码里没有引入 FTS5 的一个可能原因是：thread title 通常很短（≤ 100 字符），可以走 substring；引入 FTS5 会带来 schema migration、外置 tokenizer、SQLite 编译开关的兼容性问题，对 OpenAI 部署的多平台 binary 来说成本可能高于收益。
...
可以看出，在这五个工具里，Codex 把"事件流 + 索引 DB + 索引兜底 + 跨会话历史 + 诊断 trace"全部独立做、并显式管理跨进程并发；这反映出它是面向"CLI + VSCode + Cloud + Atlas 四端共享同一份 `~/.codex`"的多入口产品，而其它工具基本是单入口设计。
...
Codex 在桌面端用 SQLite 而非 LevelDB，可以理解为是为了支持 CLI + VSCode 插件 + app-server 三类进程同时访问同一份 `~/.codex`。SQLite 的 WAL 模式 + busy_timeout 让多进程读写安全可控，代价是写吞吐更低（5 个连接、5 秒等待）。

如果只跑单一入口，LevelDB 性能更优；但 Codex 是多入口产品，SQLite 是非常自然的选择。这一选型差异背后反映出"工具的部署形态决定了存储选型"——这是值得后续 coding agent 设计者借鉴的一条原则。
...
下图把核心数据结构按 ER 视角整理出来——可以看出 `ThreadId` 是贯穿全部三层的主要连接键。
...
- 因为有 CLI + VSCode + Atlas + Cloud 四端，所以需要 SQLite 这种跨进程安全的存储。
- 因为 VSCode 插件可能比 CLI 后启动，所以需要 backfill 把 SQLite 从历史 rollout 重建。
- 因为 backfill 可能跑很久，所以需要 lease 防止多个进程重复扫。
- 因为 backfill 有可能完不成，所以需要 fallback 路径让 list 仍然能用。
- 因为 fallback 路径需要 metadata，所以需要保留 session_index.jsonl 作为兜底索引。
- 因为多端可能同时改 thread title，所以 session_index 是 append-only + 反向扫——最后一个 entry 赢。
- 因为 rollout 文件可能被外部删，所以 list 需要 try_exists 校验。
- 因为校验后还要避免幽灵 row，所以 list 时主动 delete_thread。
- 因为 delete_thread 可能误删未来还会恢复的 row，所以 archive 不是 delete 而是改 flag。

这条因果链每一步都很小，组合起来形成了我们今天看到的约 1.6 万行复杂代码。如果 Codex 只是一个单进程 CLI，这些复杂度的 90% 都不需要存在——这也是 Aider / Goose / Continue 那些工具能保持小而美的根本原因。

换句话说，**多入口、多进程、多端共享一份本地数据**这个产品形态，是 Codex 持久化体系复杂度的真正来源。任何想要复刻 Codex 体验的项目，都应该先回答"是否真的需要多端"——如果答案是"否"，可以省掉巨大的工程投入。
...
3. **`live_thread_shutdown_with_buffered_items_materializes_before_metadata_read`**（`local/mod.rs:471-513`）：保证 shutdown 路径会 drain pending items，让 metadata 在 shutdown 后立刻可读。
...
本章用约 1.6 万行 Rust/SQL 源码实证拆开 Codex 的三层持久化体系：
