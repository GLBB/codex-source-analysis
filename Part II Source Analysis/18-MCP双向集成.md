...
合计 12 个文件、约 **8500 行 Rust 代码**，分布在 3 个 crate（`codex-mcp`、`rmcp-client`、`mcp-server`）中。可以把 Codex 的 MCP 集成总结为一句话：
...
`PendingTransport` 又是一个 4 变体枚举（`rmcp_client.rs:77-91`）：`InProcess`、`Stdio`、`StreamableHttp`、`StreamableHttpWithOAuth`。把"还没握手的 transport"与"已经握手出 service"两阶段强类型分离，是这套状态机最干净的设计：编译期就杜绝了"对 Connecting 状态调用 list_tools"或"对 Closed 状态发 initialize"。
...
最被低估的 30 行代码在 `rmcp_client.rs:172-216`：
...
这种"transport 形状统一、process 拓扑可选"的设计，让 Codex 未来可以把"远程 sandbox 内跑 MCP server"这件事接到现有 codebase 而无需改 `RmcpClient`。这是社区盲区里被忽视最重的一条。
...
5. **空 schema 的 form elicitation 会被自动 Accept**。`elicitation.rs:246-256`，这是为了"connector 想做个一键 OK 确认"留的后门，但服务器如果错把字段 schema 留空（例如 bug 导致 `properties = {}`），就会被自动批准——这是个潜在 supply chain 风险。
...
观察：Codex 在 5 个方面**明显领先**——**进程组终止**、**Streamable HTTP 唯一化**、**OS Keyring + 回退**、**Elicitation pause timer**、**Local/Executor 二分**。其中"Elicitation pause timer"与"Session 404 静默恢复"是 Codex 独家、几乎没有其他 agent 实现。Claude Code 的 plugin host 也能托管 MCP，但其 OAuth 与 stdio 终止策略远比 Codex 简陋——这与"Codex 整体 Rust 化、安全 default"的项目哲学一致。

但 Codex 在"自身作为 MCP server"上**反而**比 Claude Code 简单：`mcp-server` 只暴露 2 个工具，连 `resources/list` 都只打日志（`message_processor.rs:289-291`）。Claude Code 的 plugin host 暴露面更宽。这与社区争议 C 一致——OpenAI 选择"先把入口收窄"。

### 2.7 仍存在的问题与缺陷

1. **RFC 8707 resource indicator 长期由 rmcp 版本决定**。Codex 自己不强制 audit token audience，问题等 rmcp 修。Issue [#20729](https://github.com/openai/codex/issues/20729) 修了之后会有下一个 rmcp 兼容性问题——这是依赖外部 SDK 必然的代价。
...
- 服务端侧的"只暴露 codex 与 codex-reply 两个工具"是有意识的最小化设计，与"插件市场""resources/templates"形成生态张力；
