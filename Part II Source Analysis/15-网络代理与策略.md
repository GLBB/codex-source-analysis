...
- `codex-rs` 下 `Cargo.toml` 文件数：**120**（`find . -name Cargo.toml -not -path '*/target/*'`）
...
注意 `is_user_controlled_layer()` 在 `loader.rs:321` 把 `User / Project / SessionFlags` 三类标记为“用户可控”，其余层（system、legacy managed）才被视为可信源。这层切分的设计意图可以被理解为“防止 user-level config 偷偷放宽 system-level 边界”，但严格说，这只是源码现状的一种合理解释。
...
async fn http_connect_proxy(upgraded: Upgraded) -> Result<(), Infallible> {
    // 如果开启 CONNECT MITM 且有状态，则走 mitm_tunnel
    // 否则按 upstream proxy 或直连转发
...
最直接的好处是：“允许是 baseline allow 还是 decider override”是可观测的，而不是黑盒结果。这一点对治理侧很重要，但下游能不能利用得起来，仍取决于实际接入。
...
fn validate_absolute_form_host_header(
    ...
    // absolute-form 请求的 URI host 必须与 Host header 匹配
}
```
...
也就是：只要进入 LIMITED 模式或配置了 MITM hook，MITM 就会被自动启用。这是一种“按需开启”的取舍，避免让用户在两个独立开关之间反复对齐。
...
这一层失败发生在运行态之前，避免了请求进入“灰色状态”。代价是用户会感觉“配置明明写了却没生效”。从运维角度，把 `source_label`、层级顺序、约束冲突原因同时暴露给用户是收益最高的改进方向。
...
这层故障与安全策略无关，却最容易被误判为“策略误拦截”。`http_proxy.rs` 在响应头与日志语义上区分“策略拒绝”与“上游失败”，正是为了降低这种误诊。
