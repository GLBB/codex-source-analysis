...
- `Config` 结构体字段 114 个（运行态大总线）。
- `ConfigToml` 字段 95 个（声明态输入）。
- `ConfigRequirementsToml` 字段 19 个（治理态约束输入）。
- `ConfigLayerSource` 枚举 7 个来源层级。

这说明“配置系统”在 Codex 中已从“参数读取”演化为“运行治理内核”。
...
- **ConfigLayerStack**：做“谁说了什么”（来源合并 + 覆盖顺序）。
- **ConfigRequirements**：做“什么可以被执行”（约束、回退、拒绝）。
...
这意味着企业策略合并不是“后写覆盖前写”，而是“按来源顺序抢占空位”，设计目标是稳定与可解释，而不是动态覆盖灵活性。
...
云要求模块中有 34 个 `#[tokio::test]`，其断言直接定义了行为边界。例如：
...
本章要点不是“谁更好”，而是 Codex 在企业治理上把“策略文件”工程化成了可测试、可缓存、可追踪的内核路径。这个维度在横向竞品里并不常见，尤其是 **云策略缓存签名 + 身份绑定 + fail-closed 测试化** 这三件套。
...
很多工具把配置理解为“参数集合”，而 Codex 更接近“治理语言”：来源有层级、字段有约束、冲突有回退、失败有错误码、审计有来源标签。这个差异决定了它不是“把默认值换个地方放”，而是把“组织规则”纳入运行时决策。
...
如果把这段结构体当作“企业策略 DSL（domain-specific language）”，许多设计就解释得通了：

1. 为什么它不追求“用户越权灵活覆盖”？因为 DSL 的目标是约束表达，而不是偏好表达。  
2. 为什么它需要 `RequirementSource`？因为 DSL 必须可审计、可追责。  
3. 为什么要有 `ConstrainedWithSource<T>`？因为运行期需要知道“是谁在约束我”，不只是“我被约束了”。

这类设计在个人工具里常被视为“过重”；在企业环境里，反而是“最低可用重量”。因为一旦安全策略无法解释，最后只能回到“人工审批所有操作”，效率和风险都更差。
...
这个设计最常被忽略的价值是：**它阻断了模型与仓库内容的合谋路径**。如果项目不可信，项目配置、hooks、execpolicy 一起降级，模型即使“看到”某些仓库内策略，也无法自动把它们带入执行通道。
...
从源码注释看，要求层顺序是 cloud -> admin -> system -> legacy，且“earlier layer cannot be overridden”。这与 `merge_unset_fields` 一致，属于强一致设计，不是偶然实现细节。

对企业治理而言，这种确定性比灵活性更重要。因为一旦“后来源可随意覆盖前来源”，就会引入审计不稳定：同一用户同一仓库在不同环境变量、不同启动参数下出现不同边界，最终策略变成黑盒。
...
这意味着“本地可写文件”不会天然成为策略后门。它不是绝对防护（密钥仍在客户端代码中，存在逆向成本问题），但显著提高了低门槛篡改攻击成本。
...
把 `ConfigRequirementsToml` 按组织职责拆开看，会发现它隐含了一个标准企业分工：
...
换句话说，这个结构体不是“某个工具作者随手罗列配置项”，而是把企业内多角色协作映射成了可序列化对象。一个组织若没有这些角色分工，最终会出现“同一份 requirements 文件里塞满互相冲突意图”的情况，配置很快失控。
...
如果把当前实现看作 v1.5，下一阶段最值得投入的不是再加 20 个配置键，而是提升“证明能力”：
...
Codex 的配置系统并不是“把 `config.toml` 做厚”，而是把配置问题拆成了三层工程能力：**来源分层（谁能写）**、**约束归一（谁能改）**、**运行回退（改不了时怎么办）**。在企业场景里，这比“参数更多”更重要。源码显示它已经具备了较成熟的治理骨架：多来源 requirements、可验证缓存、路径信任门控、字段级约束与错误回退；但也仍有明显技术债：超大编排函数、文档迁移断层、历史兼容层和可观测性不足。对团队落地而言，最实用的建议是：把 `config.toml` 当“偏好层”，把 `requirements.toml` 当“策略层”，把 `AGENTS.md` 当“行为层”，三者分工明确，才能避免把模型问题误诊为配置问题。
