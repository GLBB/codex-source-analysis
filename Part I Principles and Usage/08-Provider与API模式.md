...
这些数字说明：本章并非“API 调用示例层”，而是 Codex 里最重的基础设施层之一。
...
sequenceDiagram
    ...
    Sess->>Ws: response.create(generate=false)"warmup"
    Ws->>Srv: frame1
...
%%{init: {"theme":"neutral","themeCSS":"svg { background: #ffffff !important; } .label, .nodeLabel, .edgeLabel, text { fill: #000000 !important; color: #000000 !important; }","themeVariables":{"background":"#ffffff","mainBkg":"#ffffff","primaryColor":"#f5f5f5","primaryTextColor":"#000000","primaryBorderColor":"#333333","lineColor":"#444444","secondaryColor":"#f6f8fa","tertiaryColor":"#ffffff","clusterBkg":"#fafafa","clusterBorder":"#888888","edgeLabelBackground":"#ffffff","fontFamily":"Helvetica"}}}%%
flowchart LR
    A["RealtimeWebsocketClient"] --> B["connect(api_url->ws_url)"]
...
flowchart TD
    ...
    B -->|no status426| D["Fallback to HTTP SSE"]
    B -->|no other error| E["Return mapped ApiError"]
    C --> F{"response.completed"}
    F -->|yes| G["Turn complete"]
    F -->|no timeout/close| E
...
- **Codex 的强项**：provider 与 transport 在 Rust 主仓内是“强类型 + 明确错误语义 + 可观测”的组合。`previous_response_id` 增量续跑、`response.processed`、UnauthorizedRecovery 状态机都属于工程级细节，不是文档口号。
- **Claude Code / Opencode**：在用户体验与生态接入层各有优势，但从“可读源码抽象层级”看，Codex 在 provider->transport->auth->event 的分层暴露更完整。
- **Aider / Goose / Continue**：更强调工作流接线和多模型兼容；若按“协议细粒度控制”维度比较，Codex 当前更偏“平台内核型”。

### 6.3 一个关键差异：Codex 的“降级可解释性”

Codex 在 WebSocket 失败时，有明确 fallback 或错误映射路径，而不是抽象成“网络不稳定重试一下”。这对企业环境排障非常关键：你能定位是 `upgrade required`、`401`、`429`、`connection_limit`，还是 `idle timeout`。
...
把 A/B/C/D 四条路径放在一起，能看出 Codex 的一个关键工程取向：  
**它把“模型调用”拆成了可配置、可恢复、可观测、可降级的系统工程，而不是把所有复杂性塞进一个 HTTP 请求。**
...
最后给一个实践性的判断：  
如果你的目标只是做“单轮问答”，本章机制看起来会显得重；  
但如果你的目标是做“长链路、可审计、可恢复”的编码代理，这一层复杂性不是可选项，而是必要成本。  
Codex 的价值不在于消灭复杂性，而在于把复杂性安放在可维护的边界里。
...
很多代码库也有 provider 概念，但常见实现是把 provider 做成“URL + API key”的别名。Codex 的做法明显更重：它把 capability、auth manager、models manager、account state、remote compaction 能力全部挂进 provider 抽象。  
这背后的设计意图，不是为了让代码更“学院派”，而是为了把不稳定点收拢到少量可控边界。模型后端变化很快，认证形态也会演进，如果这些变化直接渗透到 core session、tool runtime、UI 渲染层，整个系统会出现“横向污染”：任何后端变化都要改十几个子系统。  
...
社区最容易观察到的是 token 生成速度，但在编码代理场景中，端到端耗时往往并不由 token 本身主导。真实成本结构大致由四部分组成：连接建立、上下文装载、工具回路往返、模型推理。  
当任务是“一问一答”时，连接与装载成本占比不高；当任务是“20 次以上工具调用回路”时，这两项固定成本会被放大到不可忽略。Responses WebSocket 的意义就在这里：它不是让单次推理神奇变快，而是减少每轮重复支付的固定成本。  
Codex 在源码里对这个问题的处理非常工程化：先通过预连接和 warmup 降低首轮启动抖动，再通过 `previous_response_id + input delta` 降低后续轮次负担，最后用 `response.processed` 形成“处理完成”确认闭环。  
为什么要加确认闭环？因为长链路系统最怕“双方都以为对方知道”。客户端以为服务端已经确认，服务端以为客户端还在处理中，这种状态错位会在重连、重试、压缩时造成难以复盘的问题。ACK 语义就是为了解决这种“隐式假设”。  
另外一个被低估的成本是“失败语义成本”。如果系统把所有失败都折叠成 `network error`，工程团队会花大量时间在无效排查上。Codex 的 SSE/WS 映射链路在这方面做得相对细：上下文超限、配额、策略、可重试、连接上限都有分层表达，这让错误处理能接近自动化，而不是完全依赖人工阅读日志。  
可以说，Codex 在痛点层面的核心动作不是“把速度做快”，而是“把慢和错变得可解释、可预测、可恢复”。

### 三、关于“方案”的深挖：为什么是三条 API 传输面并存

从架构洁癖角度看，很多人会倾向“只保留一种传输路径”。但 Codex 实际上维护了 Responses HTTP/SSE、Responses WebSocket、Realtime WebSocket/WebRTC 三条面。  
这不是技术债，而是场景现实。  
HTTP/SSE 的优势是兼容广、基础设施成熟、接入门槛低；Responses WS 的优势是长链路 continuation；Realtime 的优势是低延迟双向媒体与事件流。三者面向的问题空间不同，强行合并反而会让每条路径都变差。  
Codex 的关键不是“并存”，而是“并存但可收敛”：  
一方面，SSE 与 WS 都尽量归一到 `ResponseEvent`；  
另一方面，session 层通过 `ModelClientSession` 对传输路径做统一协调；  
再加上 provider 声明能力边界，上层业务不会被迫理解所有底层细节。  
这是一种典型的平台化策略：在底层保留多路径自由度，在中层提供统一语义，在上层保持业务稳定。  
这也解释了为何 `core/src/client.rs` 体量这么大。它不是“请求拼装工具类”，而是多传输、多认证、多重试、多降级策略的汇合点。  
对于读者来说，理解这层以后会发现一个重要事实：Codex 的复杂度并不来源于“代码风格”，而来源于“要把同一会话语义落到多个协议现实里”。这是一种不可回避的复杂度，真正的工程价值在于把它组织好。
...
在社区讨论里，竞品比较经常陷入“谁更聪明、谁更快”的笼统结论。但从工程视角，这类比较意义有限。真正决定系统可用性的维度应该是：  
是否能在长会话下稳定续跑、是否能在失败时自动恢复、是否有清晰能力边界、是否能做跨团队协作与审计。  
按这个维度看，Codex 的优势不是“某个模型表现更好”，而是“运行时协议与恢复机制更完整”。  
这并不意味着其他工具做得差，而是意味着它们在公开实现层面的重点不同：有的更重交互体验，有的更重生态插件，有的更重轻量接线。  
因此，本章给出的比较结论不是“Codex 全面领先”，而是“在 Provider 与 API 模式这条轴上，Codex 的公开实现颗粒度更细、边界更清楚”。  
对于技术负责人来说，这个结论的价值在于选型：  
如果你要快速做一个轻量助手，可能不需要本章这套重机制；  
如果你要做企业级、可审计、可恢复、可持续演进的编码代理，本章机制几乎都是刚需。  
...
1. **Provider 在 Codex 中是运行时契约层，不是配置容器。**  
   它控制认证、能力边界、模型目录和远程能力可用性。

2. **Responses 是主线协议，WebSocket 是 continuation 优化而非简单流式替换。**  
   `previous_response_id`、增量 `input`、warmup `generate=false`、`response.processed` 共同构成了真实语义。

3. **Realtime 是独立的事件系统。**  
   它有自己的 writer/events、transcript 聚合、URL 归一化和 sideband join 逻辑。

4. **AuthManager 的恢复机制是状态机，不是粗粒度重试。**  
   account_id 一致性、单并发 refresh、永久失败缓存共同保证了可恢复性与安全边界。

5. **Codex 的优势在“工程可解释性”。**  
   当链路出错，你能明确知道是 provider 组合冲突、WebSocket 升级失败、限额、认证、还是流终止条件未满足，这比“能跑通一次”更接近生产能力。
