...
### 1.3 从 workspace 维度看：它服务的是一个 ~120-crate 系统

`codex-rs/Cargo.toml` 的 `workspace.members` 处在 **~120** 个 crate 的量级。多入口启动分发并不是"一个单体 CLI 的门面"，而是要给百级 crate 的运行面提供一致的入口与路径语义。这也是为什么本章值得独立分析。
...
Codex 需要尽量减少用户单独安装 `apply_patch` / `codex-linux-sandbox` / `codex-execve-wrapper` 等 helper 的负担，但运行时又需要这些"按名字驱动行为"的可执行。  
...
这类设计通常出现在对生命周期故障较敏感的系统里；相关源码注释也直接说明了 guard 需要保留到 async 入口结束。
