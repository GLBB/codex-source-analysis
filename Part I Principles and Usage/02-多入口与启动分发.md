...
“`codex` 是一个命令”，这是用户视角；“`codex` 是一条跨语言、跨二进制形态、跨运行模式的启动分发链”，这是工程视角。  
本章不讨论模型能力，也不讨论工具调用本身的优劣，而是聚焦一个更底层的问题：**同一个入口词 `codex`，如何在 npm 包装器、Rust 主程序、arg0 别名分发、headless `exec` 运行时之间保持行为一致、路径可追踪、错误可恢复**。

如果把 Codex 看成一个“终端 AI 工具”，你只能解释它为什么能跑；如果把它看成“多入口启动系统”，你才能解释它为什么能在 TUI、CI、Cloud、MCP、App-Server 这些不同执行面上，尽量保持同一套控制语义。

---

## 全网调研补充（近 12 个月）

> 本节基于本章专属关键词检索：`Codex CLI multi entry`、`codex npm wrapper Rust binary`、`codex arg0 dispatch`、`Codex 启动流程`，并追加了你指定平台的定向检索。下面只提炼“可用于源码分析的认知增量”，不复述教程型内容。

### 1) 权威来源覆盖情况

- **OpenAI 工程与文档（高权重）**
  - OpenAI Developers 的 CLI / Features / App-Server 文档反复强调：`codex` 是多执行面入口，`exec`、`app-server`、`mcp-server`、`cloud` 并存。
  - OpenAI 工程博文（如 App Server/Harness 相关）给出了“客户端拉起长期进程 + 协议握手 + 线程/回合生命周期”的官方叙事。
...
- **Latent Space（中高权重访谈与行业观察）**
  - 更强调 agent harness 与产品 surface 的关系：模型能力之外，入口/调度/信任机制本身就是产品竞争力。
- **Hacker News（高密度一线反馈）**
  - 争议主要集中在“Rust 重写收益到底是性能、安全、还是分发体验”，以及“CLI 名称和历史 Codex 模型命名冲突造成认知噪音”。
- **中文平台（知乎 / 少数派 / CSDN / 掘金 / 机器之心等）**
  - 近一年主流内容仍以安装、接入、报错排障为主；对 `arg0` 生命周期、PATH 注入、`exec` 输入语义的系统拆解明显不足。
...
外部文章常把 arg0 归类成“小技巧”，但从代码看它承担了 helper 可执行路径稳定性、生命周期守护、PATH 前缀管理、跨子系统路径传递，不只是 alias trick。
...
- **盲区 1：arg0 临时目录守护与 async main 生命周期耦合**  
  社区常讨论 alias 名称，不讨论“alias 目录何时失效”。而源码明确把 guard 生命周期延长到 async 入口结束，避免 helper path 提前失效。
- **盲区 2：`exec` 的 stdin 三态语义**  
  `RequiredIfPiped` / `Forced` / `OptionalAppend` 是一个很实用但鲜少被讲清的设计，直接决定 CI 管道下提示词拼接是否正确。
- **盲区 3：headless 模式默认审批策略与仓库检查策略的组合边界**  
  社区经常只谈 `--yolo`，不谈“默认 `AskForApproval::Never` + git repo check + bypass flag”的组合行为。
...
“多入口与启动分发”在 Codex 中不是边缘层，而是**执行语义一致性的入口控制层**。它做的不是“把命令跑起来”，而是把以下三件事绑定在一起：
...
这 6 个核心文件合计 **6472 行**。只看体量就能看出：启动分发在 Codex 里是“子系统”，不是“启动脚本”。

### 1.1 从入口声明看：npm 只负责“把你带到 Rust”
...
`MultitoolCli` 下的 `Subcommand` 不是几个命令，而是一个覆盖交互、自动化、协议、远程、调试的多面路由表。源码中该枚举当前包含 **24 个变体**（脚本统计），意味着“入口层”本身就是产品面。
...
`codex-rs/Cargo.toml` 的 `workspace.members` 当前为 **113**。多入口启动分发并不是“一个单体 CLI 的门面”，而是要给百级 crate 的运行面提供一致入口与路径语义。这也是为什么该章节必须独立分析。
...
`codex.js` 里维护了目标三元组到平台包的映射，当前是 **6 个目标**。这一步解决的是“安装层可用性”与“运行层确定性”。
...
痛点在于：这不是“写一次 if/else”就完了。它还要处理本地 vendor 路径、历史目录兼容、缺依赖报错引导、PATH 注入、退出码镜像。

### 2.2 难题二：一个二进制如何“模拟多个可执行工具”

Codex 不希望用户再单独安装 `apply_patch` / `codex-linux-sandbox` / `codex-execve-wrapper`，但运行时又需要这些“名字驱动行为”。  
`arg0` 子系统通过别名与 `argv[0]` 分发，解决了“单二进制部署”和“多工具行为”之间的矛盾。
...
这也是为什么 `exec/src/lib.rs` 单文件就到 **1877 行**。

### 2.4 难题四：多入口下如何保持配置与安全策略一致

`run_main` 会把 CLI 覆盖、harness 覆盖、loader 覆盖整合，再注入 `Arg0DispatchPaths`、sandbox 策略与审批策略。入口分发如果做不好，这里就会出现“同一命令不同入口行为漂移”。
...
Codex 的方案不是“一个入口接一个大 switch”，而是**四级分发链**：
...
这里的报错策略非常明确：缺平台包时不做隐式回退，而是直接失败并给重装指令，避免用户在错误架构上“带病运行”。
...
这意味着任何从 `cli` 进入的路径，默认都在 arg0 语义保护之下。
...
这不是“可有可无”的细节：当用户报告 PATH 或 helper 行为异常时，这些环境位是最直接的诊断入口。
...
#### 4.3.2 helper alias 目录构建（127 行）

`prepend_path_entry_for_codex_aliases()`（L284 起）会：

1. 在 `CODEX_HOME/tmp/arg0` 下建 session 临时目录；
2. 加锁 `.lock`，避免并发冲突；
3. 为 `apply_patch` / `applypatch` / `codex-linux-sandbox` / `codex-execve-wrapper` 创建 alias；
4. 把该目录前置到 PATH；
5. 返回 guard，延长目录生存期。
...
#### 4.4.2 `run_main`（332 行）做“配置装配 + 运行时注入”

`run_main` 的职责不是直接发请求，而是把覆盖层组装完整后再交给 `run_exec_session`。这解释了为什么它这么长：要确保 headless 运行可重复、可诊断、可继承 arg0 路径。
...
#### 4.4.3 `run_exec_session`（382 行）做“初始操作路由 + 事件循环”

关键三路：

- `ExecCommand::Review` -> `ReviewStart`
- `ExecCommand::Resume` -> `ThreadResume` 优先
- 无子命令 -> `ThreadStart + TurnStart`
...
git 仓库安全边界默认是开启的（除非显式跳过或 bypass）：
...
现实：npm 层负责分发桥接，真正执行的是 native binary。若把问题定位停在 Node 层，很多错误（sandbox/helper/path）会被误诊。

### 5.2 误区：arg0 只是别名语法糖

现实：它还承担 alias 临时目录创建、PATH 前置、helper 路径传递、guard 生命周期控制。  
尤其是这行注释已明确说明 guard 作用：
...
在 CI 中把 `echo data | codex exec "prompt"` 当作覆盖输入，常常会得到“主提示 + `<stdin>`附加块”的结果，导致提示词意图偏移。

### 5.4 隐式边界：headless 默认审批策略

`exec` 覆盖里默认 `AskForApproval::Never`（L406-L407）。这能减少自动化阻塞，但也要求你把外层环境策略（仓库隔离、网络策略、沙箱策略）配置好，否则“自动化便利”会变成“权限扩大”。

### 5.5 边界条件：`--yolo` 与 git 检查

源码明确：`dangerously_bypass_approvals_and_sandbox` 会和 git repo 检查逻辑联动（L672-L676）。  
这不是 bug，而是有意识的“外部沙箱假设”；如果团队没有外部隔离，这个假设会失效。
...
> 说明：本节只比较“入口分发与启动架构”这一维，不比较模型质量。Codex 侧结论以本章源码为准；其他工具依据公开文档与社区实践总结。
...
| helper 别名分发 | 有（arg0 + PATH alias） | 文档层不强调此机制 | 少见类似公开机制 | 无此类默认机制 | 不以 arg0 为主叙事 | 不适用（插件形态） |
| headless 独立实现 | 强（`exec` 独立 1800+ 行） | 有自动化能力 | 有批处理能力 | 有脚本化能力 | 有自动化能力 | 主要依赖 IDE 任务流 |
| 风险点 | 复杂度高，入口行为理解门槛高 | 多面统一策略复杂 | 生态碎片化风险 | 对大型组织策略层较薄 | 扩展治理复杂 | IDE 绑定强，终端独立性弱 |

### 6.1 对比结论（聚焦启动分发）

1. **Codex 的差异化在“入口工程化深度”**：不是只有命令多，而是把“分发、别名、helper、生存期、headless 契约”都做成显式代码结构。  
2. **代价是复杂度**：`cli/main.rs + exec/lib.rs + arg0/lib.rs` 三文件就接近 5900 行，这带来可维护性挑战。  
3. **适用性上，Codex 更偏“平台级 CLI”**：如果团队需要统一 TUI/CI/Cloud/MCP 入口语义，Codex 的架构更有优势；如果只要轻量问答式改代码，复杂链路未必必要。
...
- 运行时是 native 没错，但 npm 安装入口仍存在 Node 环境语义（`engines.node >=16`）。
- 对用户来说，“零依赖”常被理解为“完全不用 Node”，文档叙事与安装路径之间存在感知差异。

### 7.3 arg0 机制对运维环境有隐含前提

- 依赖 `CODEX_HOME/tmp/arg0` 可写、可锁、可创建 alias；
- PATH 修改失败时会 warning 后继续，意味着“启动成功但能力降级”是可能状态。
...
从工程语言看，启动分发层并不生产业务价值，它生产的是**契约稳定性**。契约稳定性的关键不是“功能最多”，而是以下四个边界同时稳定：

1. **命令边界**：用户输 `codex`，系统必须稳定落到同一语义域；  
2. **路径边界**：helper 可执行路径在整个进程生命周期内有效；  
3. **策略边界**：审批、沙箱、配置覆盖在不同入口不应漂移；  
4. **退出边界**：父子进程退出码和信号语义可预测。

在 Codex 里，这四个边界不是文档约定，而是代码对象：
...
这也是为什么本章的核心不是“CLI 怎么 parse 参数”，而是“**入口如何保证上下游契约不被破坏**”。只要这个契约层不稳定，任何上层能力（review、cloud、mcp）都会被入口噪音放大。

### B. 核心问题维度扩展：约束冲突矩阵

多入口分发面对的是一组典型冲突，不是单一性能问题。下面这个矩阵更贴近真实工程权衡：
...
这个矩阵的价值在于，它解释了为什么 Codex 会出现“对新手略硬、对工程团队更稳”的感受差异：系统优先级是契约稳定，不是新手最低摩擦。

### C. 解决方案维度扩展：三条工程原则

#### 原则 1：把“猜测行为”变成“显式失败”

`codex.js` 在找不到 native 包时并不尝试未知回退，而是抛出明确重装提示。这种策略短期会增加报错感知，但长期极大降低了“错平台、错路径、错权限”下的隐性故障。

#### 原则 2：把“临时路径”升级为“生命周期资源”

很多项目会把 alias 临时目录当作启动时一次性副产物；Codex 则明确把它做成带 guard 的生命周期资源，并在 async main 结束后才释放。  
这类设计通常只有在踩过真实线上故障后才会出现，说明该链路已经经过高强度实战反馈。
...
下面按“从输入到执行”的真实顺序给出逐段拆账。这个视角在排障时非常实用。
...
这一跳直接决定脚本编排系统（CI、任务调度器、守护进程）看到的终态是否可信。

#### D.5 第 4 跳：Rust 入口接入 arg0

`main()` 首行即 `arg0_dispatch_or_else`，意味着后续所有子命令路径都共享 arg0 上下文。这样做的收益是“路径契约集中处理”，而不是在每个子命令重复传递 helper path。

#### D.6 第 5 跳：arg0 快速分发

`arg0_dispatch()` 先看 `argv0`，再看 `argv1`。前者处理“可执行名触发”，后者处理“隐藏子命令触发”（如 FS helper）。  
...
这是一个很关键但常被忽略的安全点：允许 `.env` 注入普通变量，但禁止覆盖 `CODEX_` 前缀配置，避免本地环境文件意外污染核心运行参数。

#### D.8 第 7 跳：临时 alias 目录建立与 PATH 前置

这一跳决定 `apply_patch` 等 helper 是否“按名可调用”。`prepend_path_entry_for_codex_aliases` 的 127 行不是复杂化，而是把并发锁、目录权限、清理策略、跨平台差异一次处理干净。
...
`run_exec_session` 把 Review / Resume / New Turn 三路统一抽象成 `InitialOperation`。这使事件循环只关心“我要执行哪类首操作”，而不是被入口分支污染。

#### D.12 第 11 跳：输入解码与拼接

stdin 输入先做 BOM/UTF-16 检测，再决定是主提示还是附加上下文；这一步解决了自动化脚本里最常见的“编码与拼接语义”问题。
...
impl From<ResumeArgsRaw> for ResumeArgs {
    fn from(raw: ResumeArgsRaw) -> Self {
        ...
        Self { session_id, prompt, last: raw.last, all: raw.all, images: raw.images }
    }
}
```
...
- 关键指标：安装快、命令简单、交互流畅。
- Codex 的优势是入口统一和后续扩展空间；劣势是概念层稍重（subcommand/策略较多）。

#### 场景 2：团队自动化，CI 与批处理

- 关键指标：退出码可信、JSON 输出稳定、输入语义明确、可恢复。
- Codex 的 `exec` 独立运行时设计在这里有明显优势，尤其是 `--json`、schema、resume、thread 生命周期接入。

#### 场景 3：平台化组织，多入口统一治理

- 关键指标：CLI、桌面、远程、协议面是否能共享底座。
- Codex 通过 `cli + app-server + exec + arg0` 的组合，天然更接近平台型 CLI 架构；代价是维护复杂度和新成员学习成本。

因此，竞品对比的真正结论不是“绝对优劣”，而是“你处于哪个工程阶段”。多入口分发的价值只有在多执行面并存时才会完全释放。
...
这些建议并不意味着现有设计失败。相反，正因为当前启动分发已经进入“平台级复杂度”，才值得用平台级治理方法继续演进。

### H. 本章给工程团队的落地建议

如果你准备把 Codex 接入团队研发流程，建议按以下顺序落地：
...
这套顺序看似保守，但在多入口系统里非常关键：入口不稳，后面所有“智能化能力”都会被启动噪音吞噬。

---

## 小结

“多入口与启动分发”不是 Codex 的外围脚手架，而是把产品复杂度压进工程结构的第一道关口。  
它通过 npm 桥接、Rust 多工具路由、arg0 helper 机制、headless 独立运行时，把“一个命令”扩展成“同语义的多执行面系统”。这套设计带来三种直接收益：
...
`Subcommand` 的 24 个变体看起来像“命令列表”，但在运行时它们其实被拆成 5 个启动语义层。这个分层对团队治理非常重要，因为它决定了你该在哪一层做权限、审计、回归测试和发布验收。
...
这一层的核心目标是“会话连续性 + 人机交互稳定性”，通常不追求全自动，重点在于状态恢复、审批可见、上下文承接。
...
从组织治理看，自动化层应优先建立“失败可恢复”而不是“一次成功”指标。
...
很多团队会忽略维护层测试，结果是业务能力看似正常，但登录、配置、插件、版本漂移导致的生产事故频发。
...
该层不应被误当作“边缘命令”。对于大规模部署或二次封装团队，内部调试层决定了问题定位成本上限。没有这层，你就只能在黑箱里猜测行为。
...
如果不做这层分级，团队会出现一个常见问题：把所有 `codex` 行为都当“同一类命令”，结果导致策略混乱——交互命令被硬塞进自动化、自动化命令又被拿去人工调试，最终形成不可复现的问题链条。
...
17. 构建 Tokio runtime，进入 async `cli_main`。  
...
这 22 步的价值在于：你可以在任何一步截断、注入、观察，从而把“启动失败”从模糊概念变成可定位故障点。

### B.1 为什么要强调“首个 turn”而不是“命令启动成功”

很多团队把“看到欢迎输出”当作成功标准，这是不够的。  
真正的启动成功应该至少满足：
...
将这四个断点纳入团队值班手册，通常可以覆盖 80% 以上的启动类故障。
...
**经验点**：不要手工复制二进制到未知路径绕过安装，后续升级会更难维护。
...
**经验点**：这是“部分降级运行”，风险比硬失败更高，因为容易误判为“系统没问题”。
...
**经验点**：把 schema 校验前置到 CI，可以显著减少运行时失败。
...
**经验点**：有些业务场景“必须恢复旧会话”，自动回退可能违背审计要求。
...
**经验点**：如果调度器只看 stdout，不看退出码，就会导致错误重试逻辑。

---

## 附录 D：章节结论回溯（面向复盘）

为了避免“写完就忘”，这里把本章最关键的复盘结论压缩成 8 条可执行判断：

1. `codex` 不是单点入口，而是跨 npm / Rust / arg0 / runtime 的四级链路。  
2. 启动分发层的第一职责是契约稳定，不是功能堆叠。  
3. `arg0` 机制的价值在生命周期守护，不只是别名技巧。  
4. `exec` 运行时是独立工程，不是 TUI 的“无界面版”。  
5. 默认策略必须显式化，尤其在 headless 自动化场景。  
6. 排障要按事件链逐跳定位，不能只看终端表象。  
7. 团队落地必须做入口分级治理，否则策略会混乱。  
8. 启动层不稳时，上层模型与工具能力再强也会被噪音抵消。

当你把这 8 条作为评估清单去审视任意“AI coding CLI”时，会发现一个规律：真正决定长期可维护性的，往往不是模型榜单，而是启动分发层是否被当作一等工程对象。

---

## 附录 E：源码片段深度注解（逐段解释“为什么这样写”）

这一附录不再追求结构完整性，而是挑选最容易被误读的源码片段做“逐段意图解释”。对于做二次开发、企业封装、或长期维护的人来说，这部分往往比功能清单更有价值。

### E.1 `PLATFORM_PACKAGE_BY_TARGET` 不只是平台表，而是发布契约

很多读者把这段映射表看成“常规平台 if/else”。更准确的说法是：它定义了 **npm 安装层和 native 发行层之间的契约命名空间**。  
一旦这个映射变化，影响的不只是启动脚本，而是安装器、缓存、镜像、文档、故障提示甚至用户支持话术。  
因此这张表虽然只有 6 行核心映射，但它在工程上是“发布协议”，不是“实现细节”。

### E.2 `resolveNativePackage` 的双路径兼容体现迁移策略

`packageBinaryPath` 与 `legacyBinaryPath` 双路径逻辑，反映了一个重要工程原则：**升级路径要对历史布局保持可读兼容**。  
如果只保留新路径，历史安装会瞬间全部失败；如果长期保留太多历史路径，代码又会膨胀。Codex 当前做法是保留一个明确 legacy 分支，并在缺失时直接失败提示重装，这属于“短期兼容 + 中期收敛”的平衡策略。

### E.3 父进程信号镜像是脚本生态兼容的核心

很多 CLI 包装器只把子进程 stdout 透传，却不处理信号镜像。那样在 Ctrl-C、SIGTERM 场景下，调度系统看到的退出语义会不一致。  
Codex 在 `codex.js` 里主动转发信号，并在子进程因信号退出时重发同信号终结父进程。  
这对人类用户影响不明显，但对 CI、守护进程、重试系统影响极大：你能否正确重试、是否误报成功、是否正确做失败聚类，往往就靠这个语义一致性。

### E.4 `MultitoolCli` 的设计本质是“统一入口 + 分支自治”

`MultitoolCli` 把 root 级参数（config/feature/remote/profile）集中管理，然后在每个子命令分支中做“显式拒绝”和“显式继承”。  
这不是啰嗦，而是为了避免隐式传播。比如 remote 参数不是所有分支都能接受，如果你没有 `reject_remote_mode_for_subcommand` 这类守门逻辑，就会在运行期出现难以解释的行为漂移。  
因此这层看起来代码很多，实际上是在把“隐式魔法”改成“显式规则”。
...
从工程角度看，这样做可以显著降低维护面，代价是 `exec` 入口必须足够稳定且具备扩展语义。  
...
- 另一方面避免了“某个本地 `.env` 不小心重写核心系统配置”这种高风险事故。  
很多工具在这里要么全放开、要么全禁止，Codex 的做法更细粒度，也更适合企业和个人共存场景。

### E.7 alias 目录锁与 janitor 清理说明它是“活系统”

`prepend_path_entry_for_codex_aliases` 里对 lock 文件、目录权限、stale 清理的处理，说明这套机制不是一次性脚本，而是长期运行系统的一部分。  
尤其是 janitor 清理“尽量清、清不了也继续”，这是典型生产级策略：不让清理失败阻塞主流程，同时通过 warning 暴露风险。  
这类代码经常被误判为“过度工程”，但只要系统在多会话、多终端、多并发场景跑过一段时间，就会明白这些保护为什么必要。
...
如果拆得太散，排障会变成“到处找谁覆盖了谁”；如果集中太多，又会变成单点复杂。  
Codex 当前选择“集中组装 + 明确字段”，属于可维护优先而非代码短小优先。

### E.9 `thread/resume` 与 `thread/start` 的回退逻辑体现容错哲学

`run_exec_session` 在 resume 分支下先尝试解析旧线程，找不到时回退新线程启动。  
这是一种“不中断用户工作流”的容错策略。  
但它也提示我们：不同组织场景对容错要求不一样。某些合规流程要求“恢复失败就必须失败，不允许自动新建线程”。  
因此二次封装时应根据组织策略决定是否保留这种自动回退，不能机械照搬默认行为。

### E.10 stdin 解析链条体现了“自动化优先”的细致程度

`decode_prompt_bytes` 先剥 BOM，再处理 UTF-16，再处理 UTF-8；`resolve_root_prompt` 再决定是否把管道输入附加为 `<stdin>` 块。  
这些细节单看不起眼，但在真实脚本里决定了大量“怎么突然变笨了”的问题：  
很多时候模型没变、参数没变，只是输入拼接语义悄悄变了。  
Codex 把这条链显式写出来，意味着它承认自动化输入是第一公民，不把它当作交互模式的次要补丁。

### E.11 注解小结：读懂启动层，才能读懂上层能力

如果只看功能表，你会觉得“Codex 有很多命令”；  
如果读懂这些片段，你会看到另一件事：它在把“命令工具”演进成“可长期维护的执行平台”。  
这正是本章最核心的结论：  
**启动分发层是系统的地基，地基做对了，上层智能能力才不会在工程噪音里失真。**
...
4. **跳过版本治理直接追新**：入口层回归问题通常在版本切换时集中爆发。  
5. **缺少退出码治理**：只看文本输出会导致调度器重试策略错误。

### F.5 附录总结

企业落地 Codex 时，真正难的不是“模型会不会写代码”，而是“入口系统能不能被组织稳定治理”。  
本章给出的多入口与启动分发分析，本质上就是一套治理框架：  
**把命令执行从个人体验问题，升级成组织工程能力问题。**  
当团队按阶段完成这套检查表，Codex 才会从“好用工具”变成“可托管生产能力”。
...
实验目标：确认 `codex.js` 只是桥接层，不是主要执行层。  
建议做法：在可控环境中观察 `codex` 启动时的父子进程关系与退出码表现。  
你会看到：包装器负责平台选择、环境整理、信号转发；真正任务执行逻辑在 native 进程中完成。  
这组实验可以直接纠正“npm 命令 = Node 运行时主逻辑”的常见误解。

### G.2 实验二：验证 arg0 alias 生命周期

实验目标：确认 helper 路径依赖 guard 生命周期，不是静态路径。  
建议做法：启动长任务后观察 `CODEX_HOME/tmp/arg0` 下会话目录存在性，理解为何源码要把 guard 保留到 async 主流程结束。  
如果你在任务中途粗暴清理该目录，会更直观地理解 ENOENT 类故障是如何产生的。  
这组实验最适合平台工程师和运维同学，它会直接影响你如何制定清理策略。

### G.3 实验三：验证 `exec` 输入语义三态

实验目标：确认 `RequiredIfPiped`、`Forced`、`OptionalAppend` 的差异。  
建议做法：分别执行 `codex exec -`、`echo x | codex exec`、`echo x | codex exec "y"`，对比最终提示词构造。  
你会发现第三种不是覆盖，而是 `<stdin>` 附加上下文。  
这组实验对 CI 维护者价值极高，因为许多“模型突然不按预期执行”的问题，根因不是模型，而是输入拼接语义变化。

### G.4 为什么一定要做复现实验

在 agent 工具链中，文档理解和运行事实经常存在微小偏差；这些偏差在单次交互里看不出来，但在自动化、并发、长会话场景会被放大。  
复现实验的意义就是把“隐性理解”变成“显性证据”，把“口头经验”变成“可复盘流程”。  
当团队成员都经历过这三组实验后，你会明显看到协作质量提升：定位更快、争论更少、升级更稳。

### G.5 复现实验后的建议动作

完成三组实验后，不要停在“我知道了”，建议马上做三件事：  
第一，把实验步骤沉淀成团队脚本和 README，避免知识只停留在个别人脑中；  
第二，把关键观测点（目标平台识别、alias 目录、stdin 拼接结果、退出码）接入你们现有的日志或告警体系；  
第三，在每次版本升级前后都跑一遍最小实验，形成“升级前基线—升级后对比”的固定流程。  
这样做的好处是，启动分发层会从“偶发排障对象”变成“持续可验证资产”。一旦这层稳定，上层关于模型能力、工具能力、协作效率的优化才有真实落点。

再强调一次：多入口系统最怕“凭感觉改配置”。任何一次参数调整、路径改写、策略切换，都应该对应一个可复现的验证动作和一个可追溯的变更记录。只有这样，团队才能在快速迭代中保持启动层语义稳定，而不是把问题推迟到线上爆发时再被动修复。
当这套纪律形成后，启动分发层会从“隐性风险区”转变为“可经营基础设施”，这也是平台化团队最重要的长期收益之一。
