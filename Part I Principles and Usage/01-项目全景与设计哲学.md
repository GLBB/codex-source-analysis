...
...
// codex-rs/core/src/agents_md.rs:247
...
- workspace members 113（显式成员口径，不作为 crate 总数口径）；
- 仓库中 `codex-rs/**/Cargo.toml` 约 120 个；
...
| 指标 | 数值 | 说明 |
|---|---:|---|
| `codex-rs` workspace members | 113 | 来自 `codex-rs/Cargo.toml` 成员枚举；不是 crate 总数口径 |
