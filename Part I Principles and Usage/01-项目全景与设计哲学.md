...
如果把 Codex 只看成一个“会写补丁的命令行工具”，你会很快陷入两个误判：第一，误把它当作模型能力的直接外化；第二，误把它当作一个单进程、单入口、单协议的 CLI 程序。源码给出的事实刚好相反：Codex 的真正对象是“长期运行的工程执行系统”，模型只是系统中的一个可替换组件，协议与会话持久化才是稳定边界。  
...
Codex 的本质不是“一个 CLI”，而是“一个多入口、单核心、事件驱动的工程代理运行时”。  
这个结论来自三个层面的同时对齐：入口层、核心层、协议层。
...
再看 Rust 核心 README，官方直接把 Rust 实现定位成“维护主线”，并把 `core / exec / tui / cli` 视作同一工作区的关键 crate：
...
- tui/ CLI that launches a fullscreen TUI ...
- cli/ CLI multitool ...
```
...
所以，从定位上可以给出一个更精确的定义：**Codex = 单一 Rust 执行核心 + 多 surface 入口 + 协议化线程运行时 + 平台差异化沙箱后端**。
...
这直接体现了 Codex 的设计哲学：**把兼容视为成本，把演进方向写成规则**。
...
本地统计（2026-05-26）显示该枚举共 23 个子命令，体现了“单二进制多执行面”的设计取向。
...
这是非常典型的“防止能力泄漏”设计：线程句柄可用，但可变状态集中在 session loop。
...
这个细节非常关键：如果没有 pending 缓冲，流式事件会产生不可重放丢失。
...
当前基线仓库只有 2 个 AGENTS 文件（根目录 + `codex-rs/tui/src/bottom_pane/AGENTS.md`，本地统计），但实现支持层级拼接。这意味着在大仓里，行为会随 cwd 变化而变化。  
...
- 仓库中 `codex-rs/**/Cargo.toml` 共 119 个；
- `core/src/session/session.rs`、`cli/src/main.rs`、`app-server/README.md` 都是千行级以上关键文件。
...
| `codex-rs/**/Cargo.toml` 数量 | 119 | 含测试与子目录 crate manifest |
