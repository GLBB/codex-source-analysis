...
为方便对照，本研究使用的源码基线位于 `/Users/hexiaonan/workspace/formless/refer/codex`，其中 Rust 主仓 `codex-rs/` 当前包含约 120 个 crate（按 `codex-rs/**/Cargo.toml` 统计为 120，含工作区根），约 2,008 个 `.rs` 文件、约 91 万行代码（`find codex-rs -name "*.rs" -not -path "*/target/*" | xargs wc -l`）。这一规模是本调研所有"社区共识 vs 源码现实"对照的物理基础。
...
| App-Server 设计初衷 | 官方叙事强调复用 harness、支撑 IDE/桌面/Web 统一协议 | 也可能是为未来多端产品化/生态控制提前铺路 | 官方文：https://openai.com/index/unlocking-the-codex-harness/ | 官方叙事聚焦复用与稳定 API；社区推断"平台化"难以直接证伪，亦难充分证实。 |
